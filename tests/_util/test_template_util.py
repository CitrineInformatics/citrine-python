...
def _make_list_of_gems():
    faux_gems = [
        ProcessSpec(
            name="hello world",
            parameters=[
                Parameter(name="param 1", value=NominalReal(nominal=4.2, units="g")),
                Parameter(name="param 2", value=NominalCategorical(category="foo")),
                Parameter(
                    name="attr 1",
                    value=InChI(
            ...
            conditions=[Condition(name="cond 1", value=NormalReal(mean=4, std=0.5, units=""))],
        ),
        IngredientSpec(
        ...
        ProcessRun(
            ...
            parameters=[
                ...
                Parameter(name="param 3", value=NominalCategorical(category="bar")),
            ],
            conditions=[
                Condition(name="cond 1", value=NormalReal(mean=4, std=0.5, units="")),
                Condition(name="cond 2", value=NominalCategorical(category="hi")),
                Condition(
                    name="attr 1",
                    value=InChI(
        ...
        MaterialSpec(
            ...
            properties=[
                PropertyAndConditions(
                    property=Property(
                        name="prop 1", value=NormalReal(mean=100, std=10, units="g/cm**3")
                    ),
                    conditions=[Condition(name="cond 2", value=NominalCategorical(category="hi"))],
                ),
                PropertyAndConditions(
                    property=Property(name="prop 2", value=NominalReal(nominal=33, units="1/lb")),
                    conditions=[
                        Condition(name="cond 3", value=NominalCategorical(category="citrine"))
                    ],
                ),
            ],
        ),
        MeasurementSpec(
            name="meas spec 1",
            parameters=[
                Parameter(name="param 1", value=NominalReal(nominal=2.2, units="kg")),
                Parameter(name="param 2", value=NominalCategorical(category="bar")),
            ],
        ),
        MeasurementRun(
            name="meas run 1",
            spec=LinkByUID(scope="another fake scope", id="another fake id"),
            properties=[Property(name="prop 1", value=NominalReal(nominal=4.1, units=""))],
        ),
    ]
    return faux_gems
