...
def test_simple_replacement():
    ...
    json = dict(key="value", object=dict(type="material_run", uids={"my_id": "1", "id": "17"}))
    replaced_json = replace_objects_with_links(json)
    assert replaced_json == {
        "key": "value",
...
def test_nested_replacement():
    ...
    json = dict(
        object=[
            dict(type="material_run", uids={"my_id": "1"}),
    ...
    assert replaced_json == {
        "object": [
            {"type": "link_by_uid", "scope": "my_id", "id": "1"},
...
def test_failed_replacement():
    ...
    json = dict(object=dict(some_field="material_run", uids={"my_id": "1", "id": "17"}))
    assert json == replace_objects_with_links(json)  # no type field

    json = dict(object=dict(type="material_run", uids="a uid string"))
    assert json == replace_objects_with_links(json)  # uids is not a dictionary

    json = dict(object=dict(type="material_run", some_field={"my_id": "1", "id": "17"}))
    assert json == replace_objects_with_links(json)  # no uids field

    json = dict(object=dict(type="material_run", uids={}))
