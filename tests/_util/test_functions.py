...
from citrine._utils.functions import (
    get_object_id,
    validate_type,
    object_to_link_by_uid,
    rewrite_s3_links_locally,
    write_file_locally,
    migrate_deprecated_argument,
    format_escaped_url,
    MigratedClassMeta,
    generate_shared_meta,
)
...
def test_get_object_id_from_data_concepts():
    ...
    template = ConditionTemplate(
        name="test", bounds=RealBounds(0.0, 1.0, ""), uids={"id": uid}
    )
...
def test_rewrite_s3_links_locally():
    assert "http://localhost:9566" == rewrite_s3_links_locally(
        "http://localstack:4566", "http://localhost:9566"
    )
...
def test_migrated_class():
    ...
    with pytest.raises(TypeError, match="precisely"):
        ...
        class NoParent(
            deprecated_in="1.2.3", removed_in="2.0.0", metaclass=MigratedClassMeta
        ):
            pass
...
def test_recursive_subtype_recovery():
    ...
    class MigratedProperty(
        Simple, deprecated_in="1.2.3", removed_in="2.0.0", metaclass=MigratedClassMeta
    ):
        pass
...
def test_migrate_deprecated_argument():
    ...
    with pytest.warns(DeprecationWarning):
        with pytest.raises(ValueError):
            ...
            migrate_deprecated_argument(
                "something", "new name", "something else", "old name"
            )
