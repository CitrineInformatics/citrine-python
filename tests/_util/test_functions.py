...
def test_get_object_id_from_data_concepts():
    uid = str(uuid.uuid4())
    template = ConditionTemplate(name="test", bounds=RealBounds(0.0, 1.0, ""), uids={"id": uid})
...
def test_migrated_class():
    ...
    with pytest.raises(TypeError, match="precisely"):
        ...
        class NoParent(deprecated_in="1.2.3", removed_in="2.0.0", metaclass=MigratedClassMeta):
            pass
