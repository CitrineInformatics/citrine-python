...
@pytest.mark.parametrize(
    "value_type,value_value,value_serialized", VALID_SERIALIZATIONS
)
def test_mapping_property(
    key_type, value_type, key_value, value_value, key_serialized, value_serialized
):
...
def test_mapping_property_list_of_pairs(
    key_type, value_type, key_value, value_value, key_serialized, value_serialized
):
...
def test_mapping_property_list_of_pairs_multiple():
    prop = properties.Mapping(
        properties.String, properties.Integer, ser_as_list_of_pairs=True
    )
    value = {"foo": 1, "bar": 2}
...
class DummyDescriptor(object):
    ...
    map_collection_key = properties.Mapping(
        properties.Optional(properties.String), properties.Integer, "map_collection_key"
    )
    specified_mixed_list = properties.SpecifiedMixedList(
        [properties.Integer(default=100)], "specified_mixed_list"
    )
