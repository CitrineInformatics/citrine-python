...
def refresh_token(expiration: datetime = None) -> dict:
    token = jwt.encode(payload={"exp": expiration.timestamp()}, key="garbage")
...
def session():
    ...
    with requests_mock.Mocker() as m:
        m.post(
            "http://citrine-testing.fake/api/v1/tokens/refresh",
            json=token_refresh_response,
        )
        session = Session(
            refresh_token="12345", scheme="http", host="citrine-testing.fake"
        )
...
def test_session_signature(monkeypatch):
    ...
    with requests_mock.Mocker() as m:
        m.post(
            "ftp://citrine-testing.fake:8080/api/v1/tokens/refresh",
            json=token_refresh_response,
        )
    ...
    with requests_mock.Mocker() as m:
        ...
        m.post(
            f"https://{patched_host}/api/v1/tokens/refresh", json=token_refresh_response
        )
...
def test_cursor_paged_resource():
    ...
    assert (
        list(
            Session.cursor_paged_resource(
                fake_request, "foo", forward=True, per_page=10
            )
        )
        == full_result_set
    )
    assert (
        list(
            Session.cursor_paged_resource(
                fake_request, "foo", forward=True, per_page=26
            )
        )
        == full_result_set
    )
    assert (
        list(
            Session.cursor_paged_resource(
                fake_request, "foo", forward=True, per_page=40
            )
        )
        == full_result_set
    )
