...
def test_wait_while_validating(sleep_mock):
    ...
    status_detail = mock.PropertyMock(
        return_value=[StatusDetail(msg="The predictor is now validated.", level="Info")]
    )
...
def test_wait_while_executing(sleep_mock):
    ...
    status_detail = mock.PropertyMock(
        return_value=[StatusDetail(msg="Execution is complete.", level="Info")]
    )
    in_progress = mock.PropertyMock(side_effect=[True, False, False])
    ...
    wait_while_executing(
        collection=collection, execution=workflow_execution, print_status_info=True
    )
...
def test_wait_for_asynchronous_object(sleep_mock, time_mock):
    ...
    with pytest.raises(ConditionTimeoutError) as exception:
        wait_for_asynchronous_object(
            collection=collection, resource=resource, timeout=1.0
        )

    assert str(exception.value) == (
        "Timeout of 1.0 seconds reached, but task 123456 is still in progress"
    )
