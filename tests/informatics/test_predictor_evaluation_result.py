...
from citrine.informatics.predictor_evaluation_metrics import F1, PVA, RMSE
...
def test_cv_serde(example_cv_result, example_cv_result_dict):
    round_trip = PredictorEvaluationResult.build(
        json.loads(json.dumps(example_cv_result_dict))
    )
...
def test_real_pva(example_cv_result, example_real_pva_metrics):
    ...
    assert (
        example_cv_result["saltiness"]["predicted_vs_actual"][0].predicted
        == expected.predicted
    )
    assert (
        next(iter(example_cv_result["saltiness"]["predicted_vs_actual"])).actual
        == expected.actual
    )
...
def test_categorical_pva(example_cv_result, example_categorical_pva_metrics):
    ...
    assert (
        example_cv_result["salt?"]["predicted_vs_actual"][0].predicted
        == expected.predicted
    )
    assert (
        next(iter(example_cv_result["salt?"]["predicted_vs_actual"])).actual
        == expected.actual
    )
