...
from citrine.informatics.design_spaces import FormulationDesignSpace, ProductDesignSpace
from citrine.informatics.objectives import ScalarMaxObjective, ScalarMinObjective
from citrine.informatics.scores import EIScore, EVScore, LIScore

informatics_string_data = [
    (
        IngredientCountConstraint(
            formulation_descriptor=FormulationDescriptor.hierarchical(), min=0, max=1
        ),
    ...
    (
        LabelFractionConstraint(
            formulation_descriptor=FormulationDescriptor.hierarchical(), label="y", min=0, max=1
        ),
    ...
    (IntegerRangeConstraint(descriptor_key="w"), "<IntegerRangeConstraint 'w'>"),
    (
        AcceptableCategoriesConstraint(descriptor_key="x", acceptable_categories=[]),
        "<AcceptableCategoriesConstraint 'x'>",
    ),
    (
        IngredientRatioConstraint(
            formulation_descriptor=FormulationDescriptor("Flat Formulation"),
            min=0.0,
            max=1.0,
            ingredient=("x", 1.5),
            label=("x'", 0.5),
            basis_ingredients=["y", "z"],
            basis_labels=["y'", "z'"],
        ),
        "<IngredientRatioConstraint 'Flat Formulation'>",
    ),
    (
