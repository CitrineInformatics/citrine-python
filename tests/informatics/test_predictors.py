...
chain_type = CategoricalDescriptor(
    "Chain Type", categories={"Gaussian Coil", "Rigid Rod", "Worm-like"}
)
...
data_source = GemTableDataSource(
    table_id=uuid.UUID("e5c51369-8e71-4ec6-b027-1f92bdc14762"), table_version=0
)
formulation_data_source = GemTableDataSource(
    table_id=uuid.UUID("6894a181-81d2-4304-9dfa-a6c5b114d8bc"), table_version=0
)
...
def expression_predictor() -> ExpressionPredictor:
    ...
    return ExpressionPredictor(
        ...
        aliases={"Y": youngs_modulus, "v": poissons_ratio},
    )
...
def ing_to_formulation_predictor() -> IngredientsToFormulationPredictor:
    ...
    return IngredientsToFormulationPredictor(
        ...
        id_to_quantity={"water": water_quantity, "salt": salt_quantity},
...
def test_simple_report(graph_predictor):
    ...
    session = mock.Mock()
    session.get_resource.return_value = dict(
        status="OK", report=dict(descriptors=[], models=[]), uid=str(uuid.uuid4())
    )
...
def test_molecule_featurizer(molecule_featurizer):
    ...
    assert molecule_featurizer.input_descriptor == MolecularStructureDescriptor(
        "SMILES"
    )
    ...
    assert (
        str(molecule_featurizer)
        == "<MolecularStructureFeaturizer 'Molecule featurizer'>"
    )
...
def test_chemical_featurizer(chemical_featurizer):
    ...
    assert (
        str(chemical_featurizer) == "<ChemicalFormulaFeaturizer 'Chemical featurizer'>"
    )
...
def test_auto_ml_estimators():
    ...
    empty_aml = AutoMLPredictor(
        name="", description="", inputs=[x], outputs=[y], estimators={}
    )
    ...
    with pytest.raises(ValueError):
        AutoMLPredictor(
            name="", description="", inputs=[x], outputs=[y], estimators={"pancakes"}
        )
...
def test_ing_to_formulation_initialization(ing_to_formulation_predictor):
    ...
    assert ing_to_formulation_predictor.labels == {
        "solvent": {"water"},
        "solute": {"salt"},
    }
    expected_str = (
        f"<IngredientsToFormulationPredictor '{ing_to_formulation_predictor.name}'>"
    )
...
def test_mean_property_initialization(mean_property_predictor):
    ...
    assert mean_property_predictor.impute_properties is True
    assert mean_property_predictor.default_properties == {
        "density": 1.0,
        "Chain Type": "Gaussian Coil",
    }
...
def test_simple_mixture_predictor_initialization(simple_mixture_predictor):
    ...
    assert (
        simple_mixture_predictor.input_descriptor.key
        == FormulationKey.HIERARCHICAL.value
    )
...
def test_ingredient_fractions_property_initialization(ingredient_fractions_predictor):
    ...
    assert (
        ingredient_fractions_predictor.input_descriptor.key == FormulationKey.FLAT.value
    )
...
def test_attribute_accumulation_property_initialization(
    attribute_accumulation_predictor,
):
...
def test_status(graph_predictor, valid_graph_predictor_data):
    ...
    assert (
        predictor.succeeded() and not predictor.in_progress() and not predictor.failed()
    )
...
def test_feature_effects(graph_predictor):
    ...
    expected_path = (
        f"/projects/{graph_predictor._project_id}/predictors/{graph_predictor.uid}"
...
def test_feature_effects_in_progress(graph_predictor):
    feature_effects_response = FeatureEffectsResponseFactory(
        metadata__status="INPROGRESS", result=None
    )
