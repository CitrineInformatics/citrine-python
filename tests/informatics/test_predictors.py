...
def build_predictor_data(instance):
    return dict(
        name=instance.get("name"), description=instance.get("description"), instance=instance
    )
...
def test_auto_ml_estimators():
    ...
    empty_aml = AutoMLPredictor(name="", description="", inputs=[x], outputs=[y], estimators={})
    assert empty_aml.estimators == {AutoMLEstimator.RANDOM_FOREST}

    # Check passing invalid strings leads to an error
    with pytest.raises(ValueError):
        AutoMLPredictor(name="", description="", inputs=[x], outputs=[y], estimators={"pancakes"})
...
def test_single_predict(graph_predictor):
    ...
    graph_predictor.version = 2
    material_data = {
        "vars": {"X": {"m": 1.1, "s": 0.1, "type": "R"}, "Y": {"m": 2.2, "s": 0.2, "type": "R"}},
