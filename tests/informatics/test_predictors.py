...
def test_molecule_featurizer(molecule_featurizer):
    ...
    assert molecule_featurizer.dump() == build_predictor_data({
            'name': 'Molecule featurizer',
            'description': 'description',
...
def test_chemical_featurizer(chemical_featurizer):
    ...
    assert chemical_featurizer.dump() == build_predictor_data({
        'name': 'Chemical featurizer',
        'description': 'description',
...
def test_auto_ml(auto_ml):
    ...
    built = AutoMLPredictor.build(build_predictor_entity(auto_ml.dump()))
...
def test_auto_ml_no_outputs(auto_ml_no_outputs):
    ...
    built = AutoMLPredictor.build(build_predictor_entity(auto_ml_no_outputs.dump()))
...
def test_auto_ml_multiple_outputs(auto_ml_multiple_outputs):
    ...
    built = AutoMLPredictor.build(build_predictor_entity(auto_ml_multiple_outputs.dump()))
...
def test_mean_property_round_robin(mean_property_predictor):
    ...
    data = build_predictor_entity(mean_property_predictor.dump())
