...
from citrine.informatics.design_candidate import (
    ChemicalFormula,
    DesignCandidate,
    DesignMaterial,
    MeanAndStd,
...
def test_workflow_executions_without_project():
    workflow = DesignWorkflow(name="workflow", design_space_id=uuid4(), predictor_id=uuid4())
...
def test_design_material():
    values = {
        "RealValue": MeanAndStd(mean=1.4, std=0.3),
        "Cat": TopCategories(probabilities={"Red": 0.85, "Blue": 0.15}),
        "Mixture": Mixture(quantities={"Water": 0.5, "Active": 0.5}),
