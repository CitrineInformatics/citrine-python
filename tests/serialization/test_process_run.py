...
def valid_data():
    ...
    return dict(
        ...
        file_links=[],
        type="process_run",
        source={"type": "performed_source", "performed_by": "Marie Curie", "performed_date": None},
    )
...
def test_simple_deserialization(valid_data):
    ...
    assert process_run.conditions[0] == Condition(
        name="oven temp", value=NominalReal(203.0, ""), origin="measured"
    )
    ...
    assert process_run.spec == ProcessSpec(
        name="Spec for proc 1",
        tags=[],
        uids={"id": valid_data["spec"]["uids"]["id"]},
        conditions=[
            Condition(name="oven temp", value=UniformReal(175, 225, ""), origin="specified")
        ],
    )
