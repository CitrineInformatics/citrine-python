...
from citrine.informatics.reports import Report, ModelSummary
...
def test_predictor_report_build(valid_predictor_report_data):
    ...
    assert exp_model.model_settings == {"Expression": "(z) <- (x + y)"}
...
def test_empty_report_build():
    ...
    Report.build(
        dict(id="7c2dda5d-675a-41b6-829c-e485163f0e43", status="PENDING", report=None)
    )
    Report.build(
        dict(id="7c2dda5d-675a-41b6-829c-e485163f0e43", status="PENDING", report=dict())
    )
