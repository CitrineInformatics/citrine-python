...
def test_measurement_material_connection_rehydration():
    ...
    meas1 = GEMDMeasurementRun(
        "measurement on starting material", spec=meas_spec, material=starting_mat
    )

    process_spec = GEMDProcessSpec("Transformative process")
    process = GEMDProcessRun("Transformative process", spec=process_spec)
    ingredient_spec = GEMDIngredientSpec(
        name="ingredient", material=starting_mat_spec, process=process_spec
    )
    ...
    meas2 = GEMDMeasurementRun(
        "measurement on ending material", spec=meas_spec, material=ending_mat
    )
...
def test_cake():
    ...
    def _by_name(start: MaterialRun, names: Iterable[str]) -> MaterialRun | None:
