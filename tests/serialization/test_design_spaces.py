...
def test_product_deserialization(valid_product_design_space_data):
    ...
    for designSpaceClass in [ProductDesignSpace, DesignSpace]:
        ...
        assert type(design_space.dimensions[0]) is ContinuousDimension
        assert design_space.dimensions[0].lower_bound == 6.0
        assert type(design_space.dimensions[1]) is EnumeratedDimension
        assert design_space.dimensions[1].values == ["red"]
        assert type(design_space.subspaces[0]) is FormulationDesignSpace
        assert design_space.subspaces[0].uid is None
        assert type(design_space.subspaces[1]) is FormulationDesignSpace
...
def test_product_serialization(valid_product_design_space_data):
    ...
    assert (
        serialized["instance"]["subspaces"][0]
        == original_data["data"]["instance"]["subspaces"][0]
    )
    assert (
        serialized["instance"]["subspaces"][1]
        == original_data["data"]["instance"]["subspaces"][1]
    )
...
def test_enumerated_deserialization(valid_enumerated_design_space_data):
    ...
    for designSpaceClass in [DesignSpace, EnumeratedDesignSpace]:
        design_space: EnumeratedDesignSpace = designSpaceClass.build(
...
def test_enumerated_serialization_data_int_deprecated(
    valid_enumerated_design_space_data,
):
...
def test_enumerated_serialization_data_float_deprecated(
    valid_enumerated_design_space_data,
):
    design_space = EnumeratedDesignSpace.build(valid_enumerated_design_space_data)
    with pytest.deprecated_call():
...
def test_enumerated_serialization(valid_enumerated_design_space_data):
    ...
    design_space_serialization_check(
        valid_enumerated_design_space_data, EnumeratedDesignSpace
    )
...
def test_formulation_serialization(valid_formulation_design_space_data):
    ...
    design_space_serialization_check(
        valid_formulation_design_space_data, FormulationDesignSpace
    )
