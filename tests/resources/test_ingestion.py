...
def test_create_deprecated_collection(session, deprecated_dataset):
    ...
    assert session.calls == [
        FakeCall(method="GET", path=f"projects/{ingestions.project_id}")
    ]
...
def test_poll_for_job_completion_signature(ingest, operation, status, monkeypatch):
    ...
    monkeypatch.setattr(
        "citrine.resources.ingestion._poll_for_job_completion",
        _mock_poll_for_job_completion,
    )
...
def test_ingestion_flow(
    ...
    with pytest.raises(ValueError, match="UID"):
        collection.build_from_file_links(
            [FileLink(filename="mine.txt", url="http:/external.com")]
        )
    ...
    session.set_response(
        BadRequest("path", FakeRequestResponseApiError(400, "Sad face", []))
    )
