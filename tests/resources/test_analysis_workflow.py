...
def test_list_all(session, collection, base_path):
    aw_data = [
        AnalysisWorkflowEntityDataFactory(metadata__is_archived=random.choice((True, False)))
        for _ in range(5)
    ]
    ...
    expected_call = FakeCall(
        method="GET", path=base_path, params={"page": 1, "per_page": 20, "include_archived": True}
    )
...
def test_update(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(method="PUT", path=f"{base_path}/{aw_data['id']}", json=expected_payload)
    ]
    _assert_aw_equals_dict(aw, aw_data)
