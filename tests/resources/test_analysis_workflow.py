...
def _assert_aw_equals_dict(aw, aw_dict):
    ...
    _assert_user_timestamp_equals_dict(
        aw.created_by, aw.create_time, aw_dict["metadata"]["created"]
    )
    _assert_user_timestamp_equals_dict(
        aw.updated_by, aw.update_time, aw_dict["metadata"]["updated"]
    )
    ...
    if aw_dict_archived:
        _assert_user_timestamp_equals_dict(
            aw.archived_by, aw.archive_time, aw_dict_archived
        )
    else:
...
def test_register(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(method="POST", path=base_path, json=expected_payload)
    ]
...
def test_get(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(method="GET", path=f"{base_path}/{aw_data['id']}")
    ]
    _assert_aw_equals_dict(aw, aw_data)
...
def test_list_all(session, collection, base_path):
    aw_data = [
        AnalysisWorkflowEntityDataFactory(
            metadata__is_archived=random.choice((True, False))
        )
        for _ in range(5)
    ]
    ...
    expected_call = FakeCall(
        method="GET",
        path=base_path,
        params={"page": 1, "per_page": 20, "include_archived": True},
    )
...
def test_list_archived(session, collection, base_path):
    aw_data = [
        AnalysisWorkflowEntityDataFactory(metadata__is_archived=False) for _ in range(3)
    ]
    session.set_response(paging_response(*aw_data))
    ...
    expected_call = FakeCall(
        method="GET",
        path=base_path,
        params={"page": 1, "per_page": 50, "filter": "archived eq 'true'"},
    )
    assert session.calls == [expected_call]
    assert len(aws) == len(aw_data)
...
def test_list(session, collection, base_path):
    ...
    session.set_responses(
        paging_response(*aw_data[0:2]), paging_response(*aw_data[2:4])
    )
    ...
    expected_calls = [
        FakeCall(
            method="GET",
            path=base_path,
            params={"page": 1, "per_page": 2, "filter": "archived eq 'false'"},
        ),
        FakeCall(
            method="GET",
            path=base_path,
            params={"page": 2, "per_page": 2, "filter": "archived eq 'false'"},
        ),
    ]
...
def test_archive(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(method="PUT", path=f"{base_path}/{aw_data['id']}/archive", json={})
    ]
    _assert_aw_equals_dict(aw, aw_data)
...
def test_restore(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(method="PUT", path=f"{base_path}/{aw_data['id']}/restore", json={})
    ]
    _assert_aw_equals_dict(aw, aw_data)
...
def test_update(session, collection, base_path):
    ...
    assert session.calls == [
        FakeCall(
            method="PUT", path=f"{base_path}/{aw_data['id']}", json=expected_payload
        )
    ]
    _assert_aw_equals_dict(aw, aw_data)
...
def test_rebuild(session, collection, base_path):
    aw_data = AnalysisWorkflowEntityDataFactory(
        data__has_snapshot=True, metadata__has_build=True
    )
    session.set_response(aw_data)
    ...
    assert session.calls == [
        FakeCall(method="PUT", path=f"{base_path}/{aw_data['id']}/query/rerun", json={})
    ]
    _assert_aw_equals_dict(aw, aw_data)
