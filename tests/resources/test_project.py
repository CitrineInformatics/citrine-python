...
def paginated_collection(paginated_session) -> ProjectCollection:
    return ProjectCollection(session=paginated_session)
...
def test_publish_resource_deprecated(project, datasets, session):
    ...
    dataset = datasets.build(
        dict(id=dataset_id, name="public dataset", summary="test", description="test")
    )
    with pytest.deprecated_call():
...
def test_failed_register():
    ...
    session.post_resource.side_effect = NotFound(
        f"/teams/{team_id}/projects", FakeRequestResponse(400)
    )
...
def test_project_registration(collection: ProjectCollection, session):
    ...
    assert 1 == session.num_calls
    expected_call = FakeCall(
        method="POST", path=f"teams/{team_id}/projects", json={"name": "testing"}
    )
    assert expected_call == session.last_call
...
def test_list_no_team(session):
    ...
    assert 1 == session.num_calls
    expected_call = FakeCall(
        method="GET", path="/projects", params={"per_page": 1000, "page": 1}
    )
    assert expected_call == session.last_call
    assert 5 == len(projects)
...
def test_search_all(collection: ProjectCollection):
    ...
    expected_call = FakeCall(
        ...
        json={
            "search_params": {
                "name": {"value": project_name_to_match, "search_method": "EXACT"}
            }
        },
    )
...
def test_project_batch_delete_no_errors(project, session):
    ...
    # When
    with pytest.deprecated_call():
        del_resp = project.gemd_batch_delete(
