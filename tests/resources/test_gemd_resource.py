...
if __name__ == '__main__':
    ...
    #     host="development.citrine-platform.com",
    # )
    #
    # project = find_or_create_project(project_collection=client.projects, project_name="SPT-189")
    # dataset = find_or_create_dataset(dataset_collection=project.datasets, dataset_name="SPT-189")
    #
    # from gemd.demo.cake import make_cake, make_cake_spec, make_cake_templates, change_scope
    # from gemd.util import flatten
    #
    # scope = str(uuid4())
    # change_scope(scope)
    # tmpl = make_cake_templates()
    # cake_spec = make_cake_spec(tmpl=tmpl)
    # cakes = [make_cake(tmpl=tmpl, cake_spec=cake_spec) for _ in range(10)]
    # expected = flatten(cakes + list(tmpl.values()) + [cake_spec])
    #
    # result = dataset.register_all(expected, dry_run=True)
    # for obj in result:
    #     print(obj)
    #     if obj not in expected:
    #         print(obj.dump())
    #         print([x for x in expected if x.name == obj.name and isinstance(obj, type(x))])
    ...
    #         print(obj.dump())
    #         print([x for x in result if x.name == obj.name and isinstance(obj, type(x))])
