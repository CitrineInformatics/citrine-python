...
from citrine.resources.data_concepts import CITRINE_SCOPE, CITRINE_TAG_PREFIX, DataConcepts
...
def test_register_no_mutate(gemd_collection):
    """Check that register routes to the correct collections"""
    expected = {
        MaterialTemplateCollection: MaterialTemplate("foo", uids={"scope1": "A", "scope2": "B"}),
...
def test_register_all(gemd_collection):
    ...
    foo_process_template = ProcessTemplate(
        "foo", conditions=[[condition_template, bounds]], parameters=[[parameter_template, bounds]]
    )
    ...
    baz_process_template = ProcessTemplate(
        "baz", conditions=[[condition_template, bounds]], parameters=[[parameter_template, bounds]]
    )
...
def test_register_all_dry_run(gemd_collection):
    ...
    from gemd.demo.cake import change_scope, make_cake, make_cake_spec, make_cake_templates
...
def test_async_update(gemd_collection, session):
    ...
    fake_job_status_resp = {"job_type": "some_typ", "status": "Success", "tasks": [], "output": {}}
...
def test_async_update_timeout(gemd_collection, session):
    ...
    obj = ProcessTemplate("foo", uids={"id": str(uuid4())})
    fake_job_status_resp = {"job_type": "some_typ", "status": "Pending", "tasks": [], "output": {}}
...
def test_async_update_and_wait_failure(gemd_collection, session):
    ...
    obj = ProcessTemplate("foo", uids={"id": str(uuid4())})
    fake_job_status_resp = {"job_type": "some_typ", "status": "Failure", "tasks": [], "output": {}}
...
def test_batch_delete(gemd_collection, session):
        ...
        [
            {
                ...
                "cause": {
                    ...
                    "message": "",
                    "validation_errors": [
                        {"failure_message": "fail msg", "failure_id": "identifier.coreid.missing"}
                    ],
                },
            }
        ]
    ...
    # When
    del_resp = gemd_collection.batch_delete([UUID("16fd2706-8baf-433b-82eb-8c7fada847da")])
...
def test_type_passthrough(gemd_collection, session):
    ...
    metadata = {
        "dataset": str(uuid4()),
        "audit_info": AuditInfo.build({"created_by": str(uuid4()), "created_at": 1559933807392}),
        "tags": [f"{CITRINE_TAG_PREFIX}::added"],
    }
    # Set up the Condition Templates
    low_tmpl, high_tmpl = [
        ConditionTemplate(
            "condition low", uids={CITRINE_SCOPE: str(uuid4())}, bounds=IntegerBounds(1, 10)
        ),
        ConditionTemplate(
            "condition high", uids={CITRINE_SCOPE: str(uuid4())}, bounds=IntegerBounds(11, 20)
        ),
    ]
    session.set_response(
        {
