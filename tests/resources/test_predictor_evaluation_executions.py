...
def workflow_execution(
    ...
    predictor_evaluation_execution_dict,
) -> PredictorEvaluationExecution:
...
def test_build_new_execution(collection, predictor_evaluation_execution_dict):
    ...
    assert (
        execution.in_progress() and not execution.succeeded() and not execution.failed()
    )
    assert execution.status_detail


def test_workflow_execution_results(
    workflow_execution: PredictorEvaluationExecution, session, example_cv_result_dict
):
    # Given
    ...
    assert session.last_call == FakeCall(
        method="GET", path=expected_path, params={"evaluator_name": "Example Evaluator"}
    )
...
def test_trigger_workflow_execution_with_version(
    collection: PredictorEvaluationExecutionCollection,
    predictor_evaluation_execution_dict,
    session,
):
    ...
    with pytest.deprecated_call():
        actual_execution = collection.trigger(
            predictor_id, predictor_version=predictor_version
        )
...
def test_list(
    collection: PredictorEvaluationExecutionCollection, session, predictor_version
):
    ...
    expected_payload = {
        ...
        "workflow_id": str(collection.workflow_id),
        "page": 1,
    }
    if predictor_version is not None:
        expected_payload["predictor_version"] = predictor_version
    assert session.last_call == FakeCall(
        method="GET", path=expected_path, params=expected_payload
    )
...
def test_archive(workflow_execution, collection):
    ...
    expected_path = "/projects/{}/predictor-evaluation-executions/archive".format(
        collection.project_id
    )
...
def test_get(predictor_evaluation_execution_dict, workflow_execution, collection):
    ...
    with pytest.deprecated_call():
        collection.get(workflow_execution.uid)
