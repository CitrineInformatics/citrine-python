...
from tests.utils.session import FakeCall, FakeRequestResponseApiError, FakeS3Client, FakeSession
...
def test_upload(collection: FileCollection, session, tmpdir, monkeypatch):
    ...
    for dest_name in dest_names:
        ...
        url = f"teams/{collection.team_id}/datasets/{collection.dataset_id}/files/{file_id}/versions/{version}"
...
def test_list_file_links(collection: FileCollection, session, valid_data):
    ...
    assert expected_call == session.last_call
    assert len(files) == 1
    expected_url = f"teams/{collection.team_id}/datasets/{collection.dataset_id}/files/{file_id}/versions/{version}"
...
def test_read(collection: FileCollection, session, tmp_path):
    ...
    filelink = collection.build(
        {"id": str(uuid4()), "version": str(uuid4()), "filename": filename, "type": FileLink.typ}
    )
...
def test_ingest_with_upload(collection, monkeypatch, tmp_path, session):
    ...
    def _mock_build_from_file_links(
        self: IngestionCollection, file_links: Collection[FileLink], *, raise_errors: bool = True
    ):
    ...
    session.set_response(NotFound("path", FakeRequestResponseApiError(400, "Not found", [])))
