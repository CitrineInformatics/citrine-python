...
def test_list(valid_graph_predictor_data, valid_graph_predictor_data_empty):
    ...
    expected_call = FakeCall(
        method="GET",
        path=f"/projects/{collection.project_id}/predictors",
...
def test_check_update_none():
    ...
    expected_call = FakeCall(
        method="GET", path=f"/projects/{pc.project_id}/predictors/{predictor_id}/update-check"
    )
...
def test_returned_predictor(valid_graph_predictor_data):
    ...
    result = pc.create_default(
        training_data=GemTableDataSource(table_id=uuid.uuid4(), table_version=0), pattern="PLAIN"
    )
...
def test_is_stale(valid_graph_predictor_data, is_stale):
    ...
    response = {"id": pred_id, "version": pred_version, "status": "READY", "is_stale": is_stale}
...
def test_retrain_stale(valid_graph_predictor_data):
    ...
    assert session.calls == [
        FakeCall(method="PUT", path=f"{versions_path}/{pred_version}/retrain-stale", json={})
    ]
...
def test_create_default_async():
    ...
    assert session.calls == [
        FakeCall(method="POST", path=f"{predictors_path}/default-async", json=expected_payload)
    ]
...
def test_rename(valid_graph_predictor_data):
    ...
    assert session.calls == [
        FakeCall(
            method="PUT", path=f"{versions_path}/{pred_version}/rename", json=expected_payload
        )
    ]
