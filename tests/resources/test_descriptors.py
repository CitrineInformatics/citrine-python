...
from citrine.informatics.predictors import GraphPredictor, MolecularStructureFeaturizer
...
def test_from_predictor_responses():
    ...
    response_json = {
        "responses": [  # shortened sample response
            {
                "type": "Real",
                "descriptor_key": f"khs.sNH3 KierHallSmarts for {col}",
                "units": "",
                "lower_bound": 0,
            ...
            {
                "type": "Real",
                "descriptor_key": f"khs.dsN KierHallSmarts for {col}",
                "units": "",
                "lower_bound": 0,
                "upper_bound": 1000000000,
            },
    ...
    results = descriptors.from_predictor_responses(
        predictor=featurizer, inputs=[MolecularStructureDescriptor(col)]
    )
    assert results == [
        RealDescriptor(
    ...
    graph = GraphPredictor(
        name="Graph", description="Contains a featurizer", predictors=[featurizer]
    )
    graph_results = descriptors.from_predictor_responses(
        predictor=graph, inputs=[MolecularStructureDescriptor(col)]
    )
...
def test_from_data_source():
    ...
    data_source = GemTableDataSource(
        table_id="43357a66-3644-4959-8115-77b2630aca45", table_version=123
    )

    results = descriptors.from_data_source(data_source=data_source)
    assert results == [
        RealDescriptor(
            key=r["descriptor_key"],
            lower_bound=r["lower_bound"],
