...
def test_object_template_validation():
    ...
    color_template = ParameterTemplate(
        "Color", bounds=CategoricalBounds(["red", "green", "blue"])
    )
    ...
    with pytest.raises(ValueError):
        MaterialTemplate(
            "Block", properties=[[length_template, RealBounds(3.0, 4.0, "cm")]]
        )

    with pytest.raises(ValueError):
        ProcessTemplate(
            "a process", conditions=[[color_template, CategoricalBounds(["zz"])]]
        )

    with pytest.raises(ValueError):
        MeasurementTemplate(
            "A measurement", parameters=[[dial_template, IntegerBounds(-3, -1)]]
        )
...
def test_template_assignment():
    ...
    template = ProcessTemplate(
        "Dry", conditions=[[humidity_template, RealBounds(0.5, 0.65, "")]]
    )
    ProcessSpec(
        "Dry a polymer",
        template=template,
        conditions=[
            Condition(
                "Humidity", value=NominalReal(0.6, ""), template=humidity_template
            )
        ],
    )
...
def test_automatic_async_update():
    ...
    session = FakeSession()
    collection = PropertyTemplateCollection(
        team_id=uuid4(), dataset_id=uuid4(), session=session
    )
    this_id = str(uuid4())
    template = PropertyTemplate(
        "dummy template", bounds=RealBounds(0.0, 0.5, ""), uids={"id": this_id}
    )
...
def test_process_template_equals():
    ...
    from citrine.resources.process_template import (
        ProcessTemplate as CitrineProcessTemplate,
    )
    from gemd.entity.template import ProcessTemplate as GEMDProcessTemplate

    gemd_obj = GEMDProcessTemplate(name="My Name", tags=["tag!"])
    citrine_obj = CitrineProcessTemplate(name="My Name", tags=["tag!"])
...
def test_material_template_equals():
    ...
    from citrine.resources.material_template import (
        MaterialTemplate as CitrineMaterialTemplate,
    )
    from gemd.entity.template import MaterialTemplate as GEMDMaterialTemplate

    gemd_obj = GEMDMaterialTemplate(name="My Name", tags=["tag!"])
    citrine_obj = CitrineMaterialTemplate(name="My Name", tags=["tag!"])
    assert gemd_obj == citrine_obj, "GEMD/Citrine equivalence"
    citrine_obj.name = "Something else"
    assert gemd_obj != citrine_obj, "GEMD/Citrine detects difference"
...
def test_measurement_template_equals():
    ...
    from citrine.resources.measurement_template import (
        MeasurementTemplate as CitrineMeasurementTemplate,
    )
    from gemd.entity.template import MeasurementTemplate as GEMDMeasurementTemplate

    gemd_obj = GEMDMeasurementTemplate(name="My Name", tags=["tag!"])
    citrine_obj = CitrineMeasurementTemplate(name="My Name", tags=["tag!"])
    assert gemd_obj == citrine_obj, "GEMD/Citrine equivalence"
    citrine_obj.name = "Something else"
    assert gemd_obj != citrine_obj, "GEMD/Citrine detects difference"
...
def test_parameter_template_equals():
    ...
    from citrine.resources.parameter_template import (
        ParameterTemplate as CitrineParameterTemplate,
    )
...
def test_property_template_equals():
    """Test basic equality.  Complex relationships are tested in test_material_run.test_deep_equals()."""
    from citrine.resources.property_template import (
        PropertyTemplate as CitrinePropertyTemplate,
    )
