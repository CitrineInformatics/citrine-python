...
from citrine.resources.material_spec import (
    MaterialSpec as CitrineMaterialSpec,
...
def test_repeat_serialization_gemd(collection, session):
    ...
    proc = GEMDProcess(
        name="Test gemd mutation (process)", uids={"nomutate": "process"}
    )
    mat = GEMDMaterial(
        name="Test gemd mutation", uids={"nomutate": "material"}, process=proc
    )
...
def test_equals():
    gemd_obj = GEMDMaterialSpec(name="My Name", notes="I have notes", tags=["tag!"])
