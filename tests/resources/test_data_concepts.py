...
def run_noop_gemd_relation_search_test(
    search_for, search_with, collection, search_fn, per_page=100
):
    ...
    assert collection.session.last_call == FakeCall(
        method="GET",
        path="teams/{}/{}/{}/{}/{}".format(
            collection.team_id, search_with, test_scope, test_id, search_for
        ),
        params={
...
def test_assign_audit_info():
    ...
    assert sample_object.audit_info == audit_info_obj, (
        "Audit info should be built from a dict"
    )

    another_object = ProcessSpec.build(
        {"type": "process_spec", "name": "A process spec", "audit_info": audit_info_obj}
    )
    assert another_object.audit_info == audit_info_obj, (
        "Audit info should be built from an obj"
    )
...
def test_make_link_by_uid():
    ...
    spec = ProcessSpec(
        "spec", uids={"custom scope": "custom id", CITRINE_SCOPE: str(uid)}
    )
    ...
    assert _make_link_by_uid(no_citrine_id) == LinkByUID(
        scope="custom scope", id="custom id"
    )
