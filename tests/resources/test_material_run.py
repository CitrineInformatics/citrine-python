...
def test_get_history(collection, session):
    ...
    root_obj = next(
        o for o in cake_json["context"] if root_link.id == o["uids"].get(root_link.scope)
    )
...
def test_get_material_run(collection, session):
    ...
    expected_call = FakeCall(
        method="GET",
        path=f"teams/{collection.team_id}/datasets/{collection.dataset_id}/material-runs/id/{mr_id}",
    )
    assert expected_call == session.last_call
...
def test_list_material_runs(collection, session):
    ...
    expected_call = FakeCall(
        method="GET",
        path=f"teams/{collection.team_id}/material-runs",
        params={
...
def test_cursor_paginated_searches(collection, session):
    ...
    """
    all_runs = [MaterialRunDataFactory(name=f"foo_{i}") for i in range(20)]
    ...
    session.get_resource = fake_request
    session.post_resource = fake_request
    session.cursor_paged_resource = Session.cursor_paged_resource
...
def test_delete_material_run(collection, session):
    ...
    expected_call = FakeCall(
        method="DELETE",
        path=f"teams/{collection.team_id}/datasets/{collection.dataset_id}/material-runs/{material_run_scope}/{material_run_uid}",
...
def test_material_run_can_get_with_no_id(collection, session):
    ...
    assert 1 == session.num_calls
    expected_call = FakeCall(
        method="GET", path=f"teams/{collection.team_id}/material-runs/id/{mr_id}"
    )
    assert expected_call == session.last_call
    assert "Cake 2" == run.name
...
def test_validate_templates_successful_minimal_params(collection, session):
    ...
    expected_call = FakeCall(
        method="PUT",
        path=f"teams/{team_id}/material-runs/validate-templates",
...
def test_validate_templates_unrelated_400_with_api_error(collection, session):
    ...
    # When
    session.set_response(
        BadRequest("path", FakeRequestResponseApiError(400, "I am not a validation error", []))
    )
    with pytest.raises(BadRequest):
        collection.validate_templates(model=run)
...
def test_equals():
    ...
    citrine_obj = CitrineMaterialRun(name="My Name", notes="I have notes", tags=["tag!"])
