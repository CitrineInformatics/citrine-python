...
def test_get_report():
    ...
    session.set_response(
        dict(status="PENDING", report=dict(descriptors=[], models=[]), uid=str(str(uuid.uuid4())))
    )
...
def test_get_report_with_version():
    ...
    report_path = (
        f"/projects/{project_id}/predictors/{predictor_id}/versions/{predictor_version}/report"
    )
    ...
    report = ReportResource(project_id, session).get(
