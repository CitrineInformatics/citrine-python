...
def assert_erds_csv(erds_csv, erds_dict):
    for row, expt in zip(
        csv.DictReader(io.StringIO(erds_csv)), erds_dict["data"]["experiments"]
    ):
...
def test_build(collection):
    ...
    assert (
        int(actual_erds.create_time.timestamp() * 1000 + 0.0001)
        == erds_dict["metadata"]["created"]["time"]
    )

    for actual_experiment, erds_experiment in zip(
        actual_erds.experiments, erds_dict["data"]["experiments"]
    ):
        ...
        # TODO: It'd be better to actually invoke the Datetime._serialize method
        assert (
            int(actual_experiment.updated_time.timestamp() * 1000 + 0.0001)
            == erds_experiment["updated_time"]
        )

        for actual_override, erds_override in zip(
            actual_experiment.overrides.items(), erds_experiment["overrides"].items()
        ):
...
def test_list(session, collection, erds_base_path):
    ...
    assert session.calls == [
        FakeCall(
            method="GET", path=erds_base_path, params={"per_page": 100, "page": 1}
        ),
        FakeCall(
            method="GET",
            path=erds_base_path,
            params={"per_page": 100, "branch": str(version_id), "page": 1},
        ),
        FakeCall(
            method="GET",
            path=erds_base_path,
            params={"per_page": 100, "version": 4, "page": 1},
        ),
        FakeCall(
            method="GET",
            path=erds_base_path,
            params={"per_page": 100, "version": LATEST_VER, "page": 1},
        ),
        FakeCall(
            method="GET",
            path=erds_base_path,
