...
from citrine.resources.team import READ, SHARE, WRITE, Team, TeamCollection, TeamMember
from citrine.resources.user import User
from tests.utils.factories import TeamDataFactory, UserDataFactory
...
def test_team_registration(collection: TeamCollection, session):
    ...
    expected_call_3 = FakeCall(
        method="PUT",
        path=f"/teams/{created_team.uid}/users",
...
def test_list_members(team, session):
    ...
    # Then
    assert 1 == session.num_calls
    expect_call = FakeCall(method="GET", path=f"/teams/{team.uid}/users")
...
def test_update_user_actions(team, session):
    ...
    # When
    update_user_role_response = team.update_user_action(
        user_id=User.build(user), actions=[WRITE, SHARE]
    )
...
def test_add_user(team, session):
    ...
    assert 1 == session.num_calls
    expect_call = FakeCall(
        method="PUT", path=f"/teams/{team.uid}/users", json={"id": user["id"], "actions": ["READ"]}
    )
    assert expect_call == session.last_call
...
def test_add_user_with_actions(team, session):
    ...
    expect_call = FakeCall(
        method="PUT",
        path=f"/teams/{team.uid}/users",
        json={"id": user["id"], "actions": ["READ", "WRITE"]},
    )
    assert expect_call == session.last_call
    assert add_user_response is True
...
def test_remove_user(team, session):
    ...
    assert 1 == session.num_calls
    expect_call = FakeCall(
        method="POST", path=f"/teams/{team.uid}/users/batch-remove", json={"ids": [user["id"]]}
    )
    assert expect_call == session.last_call
...
def test_share(team, other_team, session):
    ...
    expect_call = FakeCall(
        method="POST",
        path=f"/teams/{team.uid}/shared-resources",
        json={
...
def test_un_share(team, other_team, session):
    ...
    expect_call = FakeCall(
        ...
        json={"target_team_id": str(other_team.uid)},
    )
    assert expect_call == session.last_call
    assert share_response is True
...
def test_list_resource_ids(team, session, resource_type, method):
    ...
    assert session.calls[0] == FakeCall(
        method="GET",
    ...
    assert session.calls[2] == FakeCall(
        method="GET",
        path=f"/{resource_type.value}/authorized-ids",
...
def test_owned_dataset_ids(team):
    ...
    for d_id in ids:
        dataset = Dataset(
            name=f"Test Dataset - {d_id}", summary="Test Dataset", description="Test Dataset"
        )
...
def test_team_batch_delete_no_errors(team, session):
    ...
    successful_job_resp = {
        ...
        "tasks": [
            {
                "id": "7b6bafd9-f32a-4567-b54c-7ce594edc018",
