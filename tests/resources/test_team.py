...
def test_team_member_string_representation(team):
    ...
    assert team_member.__str__() == "<TeamMember {!r} can {!s} in {!r}>".format(
        user.screen_name, team_member.actions, team.name
    )
...
def test_team_registration(collection: TeamCollection, session):
    ...
    expected_call_3 = FakeCall(
        method="PUT",
        path="/teams/{}/users".format(created_team.uid),
...
def test_list_teams(collection, session):
    ...
    assert 1 == session.num_calls
    expected_call = FakeCall(
        method="GET", path="/teams", params={"per_page": 100, "page": 1}
    )
    assert expected_call == session.last_call
    assert 5 == len(teams)
...
def test_me(team, session):
    ...
    member_call = FakeCall(
        method="GET", path="/teams/{}/users/{}".format(team.uid, user["id"])
    )
...
def test_update_user_actions(team, session):
    ...
    expect_call = FakeCall(
        method="PUT",
        path="/teams/{}/users".format(team.uid),
...
def test_un_share(team, other_team, session):
    ...
    expect_call = FakeCall(
        ...
        path="/teams/{}/shared-resources/{}/{}".format(
            team.uid, "DATASET", str(dataset.uid)
        ),
...
def test_team_batch_delete_no_errors(team, session):
    ...
    # When
    del_resp = team.gemd_batch_delete(
    ...
    session.set_responses(job_resp, successful_job_resp)
    entity = ProcessSpec(
        name="proc spec", uids={"id": "16fd2706-8baf-433b-82eb-8c7fada847da"}
    )
