...
from citrine.informatics.descriptors import FormulationKey, RealDescriptor
...
def _ds_dict_to_response(ds_dict, status="CREATED"):
    ...
    return {
        ...
        "metadata": {
            "created": {"user": str(uuid.uuid4()), "time": time},
            "updated": {"user": str(uuid.uuid4()), "time": time},
...
def test_design_space_build(valid_product_design_space_data):
    ...
    assert (
        design_space.name == valid_product_design_space_data["data"]["instance"]["name"]
    )
    assert (
        design_space.dimensions[0].descriptor.key
        == valid_product_design_space_data["data"]["instance"]["dimensions"][0][
            "descriptor"
        ]["descriptor_key"]
    )
...
def test_design_space_build_with_status_detail(valid_product_design_space_data):
    ...
    status_detail_tuples = {
        (detail.level, detail.msg) for detail in design_space.status_detail
    }
...
def test_convert_to_hierarchical(valid_hierarchical_design_space_data):
    ...
    dc.convert_to_hierarchical(
        uid=ds_id, predictor_id=predictor_id, predictor_version=2
    )

    expected_payload = {"predictor_id": str(predictor_id), "predictor_version": 2}
...
def test_design_space_limits():
    ...
    descriptors = [
        RealDescriptor(f"R-{i}", lower_bound=0, upper_bound=1, units="")
        for i in range(128)
    ]
...
def test_create_default(predictor_version, valid_product_design_space):
    ...
    expected_response = {
        **valid_product_design_space.dump(),
        "settings": expected_payload,
    }
    assert default_design_space.dump() == expected_response


@pytest.mark.parametrize("predictor_version", (2, "1", "latest", None))
def test_create_default_hierarchical(
    predictor_version, valid_hierarchical_design_space_data
):
    valid_hierarchical_design_space = HierarchicalDesignSpace.build(
    ...
    expected_response = {
        **valid_hierarchical_design_space.dump(),
        "settings": expected_payload,
    }
    assert default_design_space.dump() == expected_response
...
def test_create_default_with_config(
...
def test_list_design_spaces(
    valid_product_design_space_data, valid_hierarchical_design_space_data
):
    # Given
    session = FakeSession()
    collection = DesignSpaceCollection(uuid.uuid4(), session)
    ...
    expected_call = FakeCall(
        method="GET",
        path="/projects/{}/design-spaces".format(collection.project_id),
...
def test_list_all_design_spaces(
    valid_product_design_space_data, valid_hierarchical_design_space_data
):
    # Given
    session = FakeSession()
...
def test_list_archived_design_spaces(
    valid_product_design_space_data, valid_hierarchical_design_space_data
):
    # Given
    session = FakeSession()
...
def test_carrying_settings_from_create_default(valid_product_design_space):
    ...
    collection.register(default_design_space)
    ...
    expected_payload = {
        **valid_product_design_space.dump(),
        "settings": expected_settings.dump(),
    }
