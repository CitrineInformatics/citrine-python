...
from citrine.seeding.find_or_create import (
    create_or_update,
    find_collection,
    find_or_create_dataset,
    find_or_create_project,
    find_or_create_team,
    get_by_name_or_create,
...
def fake_collection() -> Collection:
    ...
    collection = FakeCollection(
        dataset_id=UUID("6b608f78-e341-422c-8076-35adc8828545"),
...
def project_collection() -> Callable[[bool], ProjectCollection]:
    ...
    def _make_project(search_implemented: bool = True, team_id: UUID | str | None = uuid4()):
...
def dataset_collection() -> DatasetCollection:
    datasets = FakeDatasetCollection(
        team_id=UUID("6b608f78-e341-422c-8076-35adc8828545"), session=FakeSession()
    )
    for i in range(5):
        num_string = str(i)
        datasets.register(
            Dataset(
                "dataset " + num_string,
                summary="summ " + num_string,
                description="desc " + num_string,
            )
        )
    for i in range(2):
...
def predictor_collection() -> PredictorCollection:
    predictors = FakePredictorCollection(
        UUID("6b608f78-e341-422c-8076-35adc8828545"), FakeSession()
    )
...
def test_get_by_name_or_create_no_exist(fake_collection):
    ...
    result = get_by_name_or_create(
        collection=fake_collection, name="New Resource", default_provider=default_provider
    )
...
def test_get_by_name_or_create_exist(fake_collection):
    ...
    old_resource_count = len(list(fake_collection.list()))
    result = get_by_name_or_create(
        collection=fake_collection, name=resource_name, default_provider=default_provider
    )
    new_resource_count = len(list(fake_collection.list()))
...
def test_find_or_create_raise_error_team_no_exist(team_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_team(
            team_collection=team_collection(), team_name=absent_name, raise_error=True
        )
...
def test_find_or_create_project_exist_multiple(project_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_project(
            project_collection=project_collection(), project_name=duplicate_name
        )
...
def test_find_or_create_raise_error_project_no_exist(project_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_project(
            project_collection=project_collection(), project_name=absent_name, raise_error=True
        )
...
def test_find_or_create_raise_error_project_exist(project_collection):
    ...
    result = find_or_create_project(
        project_collection=collection, project_name="project 3", raise_error=True
    )
    new_project_count = len(list(collection.list()))
...
def test_find_or_create_raise_error_project_exist_multiple(project_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_project(
            project_collection=project_collection(), project_name=duplicate_name, raise_error=True
        )
...
def test_find_or_create_dataset_exist(dataset_collection):
    ...
    old_dataset_count = len(list(dataset_collection.list()))
    result = find_or_create_dataset(
        dataset_collection=dataset_collection, dataset_name="dataset 2"
    )
    new_dataset_count = len(list(dataset_collection.list()))
...
def test_find_or_create_dataset_raise_error_no_exist(dataset_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_dataset(
            dataset_collection=dataset_collection, dataset_name=absent_name, raise_error=True
        )
...
def test_find_or_create_dataset_raise_error_exist(dataset_collection):
    ...
    old_dataset_count = len(list(dataset_collection.list()))
    result = find_or_create_dataset(
        dataset_collection=dataset_collection, dataset_name="dataset 3", raise_error=True
    )
    new_dataset_count = len(list(dataset_collection.list()))
...
def test_find_or_create_dataset_raise_error_exist_multiple(dataset_collection):
    ...
    with pytest.raises(ValueError):
        find_or_create_dataset(
            dataset_collection=dataset_collection, dataset_name=duplicate_name, raise_error=True
        )
...
def test_create_or_update_unique_found_design_workflow(session):
    ...
    collection = LocalDesignWorkflowCollection(
        project_id=uuid4(), session=session, branch_root_id=root_id, branch_version=version
    )
