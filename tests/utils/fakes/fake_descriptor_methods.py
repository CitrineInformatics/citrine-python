...
from citrine.informatics.descriptors import CategoricalDescriptor, Descriptor, RealDescriptor
...
class FakeDescriptorMethods(DescriptorMethods):
    ...
    def from_predictor_responses(
        self, predictor: PredictorNode | GraphPredictor, inputs: list[Descriptor]
    ):
        if isinstance(predictor, (MolecularStructureFeaturizer, ChemicalFormulaFeaturizer)):
            ...
            return [
                ...
                CategoricalDescriptor(
                    f"{input_descriptor.key} categorical property", categories=["cat1", "cat2"]
                )
            ]
