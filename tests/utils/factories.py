...
class PropertiesCriteriaDataFactory(factory.DictFactory):
    ...
    class Params:
        integer = factory.Trait(value_type_filter=factory.SubFactory(IntegerFilterDataFactory))
        category = factory.Trait(value_type_filter=factory.SubFactory(CategoryFilterDataFactory))
...
class MaterialRunClassificationCriteriaDataFactory(factory.DictFactory):
    ...
    classifications = factory.Faker(
        "random_elements", elements=[str(x) for x in MaterialClassification], unique=True
    )
