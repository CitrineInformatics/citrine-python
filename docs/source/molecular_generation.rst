...
At the end of the generation phase, the mutations are filtered based on their similarity to the starting seed molecules. Mutations that do not meet the similarity threshold (or are duplicates) are discarded. The result is a subset of the original mutations returned as list of :class:`~citrine.informatics.generative_design.GenerativeDesignResult`s. Like the input, these results contain information about the seed molecule, the mutation, the similarity score, and the fingerprint type used in the execution.
