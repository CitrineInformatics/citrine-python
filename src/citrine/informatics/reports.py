...
class FeatureImportanceReport(Serializable["FeatureImportanceReport"]):
    ...
    def __str__(self):
        return "<FeatureImportanceReport {!r}>".format(
            self.output_key
        )  # pragma: no cover
...
class ModelSummary(Serializable["ModelSummary"]):
    ...
    predictor_uid = properties.Optional(
        properties.UUID(), "predictor_configuration_uid"
    )
...
class PredictorReport(Serializable["PredictorReport"], Report):
    ...
    descriptors = properties.List(
        properties.Object(Descriptor), "report.descriptors", default=[]
    )
    """:List[Descriptor]: All descriptors that appear in the predictor"""
    model_summaries = properties.List(
        properties.Object(ModelSummary), "report.models", default=[]
    )
    ...
    def post_build(self):
        ...
        for _, summary in enumerate(self.model_summaries):
            ...
            summary.model_settings = self._collapse_model_settings(
            ...
            if summary.selection_summary is not None:
                ...
                for result in summary.selection_summary.evaluation_results:
                    result.model_settings = self._collapse_model_settings(
    ...
    def _fill_out_descriptors(self):
        ...
        for key, group in groupby(
            sorted(self.descriptors, key=lambda d: d.key), lambda d: d.key
        ):
        ...
        for i, model in enumerate(self.model_summaries):
            for j, input_key in enumerate(model.inputs):
                try:
                    ...
                    raise RuntimeError(
                        "Model {} contains input '{}', but no descriptor found "
            ...
            for j, output_key in enumerate(model.outputs):
                try:
                    ...
                    raise RuntimeError(
                        "Model {} contains output '{}', but no descriptor found "
    ...
    def _get_sole_descriptor(it: Iterable):
        ...
        if len(as_list) > 1:
            ...
            logger.warning(
                "Warning: found multiple descriptors with the key '{}', arbitrarily "
                "selecting the first one. The descriptors are: {}".format(
                    as_list[0].key, serialized_descriptors
                )
            )
        return as_list[0]
