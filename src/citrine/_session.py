...
class Session(requests.Session):
    ...
    def __init__(
        ...
        *,
        ...
        if host is None:
            ...
            if host is None:
                raise ValueError(
                    "No host passed and environmental variable CITRINE_API_HOST not set."
                )
        ...
        agent = (
            ...
            f"citrine-python/{citrine.__version__}"
        )

        # Following scheme:[//authority]path[?query][#fragment] (https://en.wikipedia.org/wiki/URL)
        self.headers.update({"Content-Type": "application/json", "User-Agent": agent})
    ...
    def _request_with_retry(self, method, uri, **kwargs):
        ...
        try:
            ...
            logger.warning(f"{e!r} seen, retrying request")
            response = self.request(method, uri, **kwargs)

        return response

    def checked_request(
        self, method: str, path: str, version: str = "v1", **kwargs
    ) -> requests.Response:
        ...
        if 200 <= response.status_code <= 299:
            ...
            if response.status_code == 400:
                ...
                logger.debug("%s %s %s", response.status_code, method, path)
                msg = f"Cant execute at this time. Try again later. Error: {response.text}"
    ...
    def _extract_response_json(path, response) -> dict:
        ...
        try:
            ...
            logger.info(
                "Response at path %s with status code %s failed json parsing with"
