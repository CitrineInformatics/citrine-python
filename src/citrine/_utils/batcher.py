...
class Batcher(ABC):
    ...
    @abstractmethod
    def batch(
        self, objects: Iterable[DataConcepts], batch_size
    ) -> List[List[DataConcepts]]:
...
class BatchByType(Batcher):
    ...
    def batch(
        ...
        for obj in objects:
            if (
                obj.to_link() in seen
            ):  # Repeat in the iterable; don't add it to the batch
        ...
        typ_groups = sorted(
            list(by_type.values()), key=lambda x: writable_sort_order(x[0])
        )
    ...
    def batch(
        ...
        for type_group in type_groups:
            for obj in type_group:
                ...
                local_set = {
                    index.get(x, x) for x in depends[obj] if index.get(x, x) in obj_set
                }
                full_set = set(local_set)
                if len(full_set) > batch_size:
                    raise ValueError(
                        f"Object {obj.name} has more than {batch_size} dependencies."
                    )

                for subobj in local_set:
                    full_set.update(depends[subobj])
                depends[obj] = sorted(
                    list(full_set), key=lambda x: writable_sort_order(x)
                )
