...
def _get_ids_from_url(url: str) -> Tuple[Optional[UUID], Optional[UUID]]:
    ...
    if (
        len(split_path) >= 4
        and split_path[-4] == "files"
        and split_path[-2] == "versions"
    ):
...
class FileLink(
    ...
    created_time = properties.Optional(
        properties.Datetime, "created_time", serializable=False
    )
    ...
    version_number = properties.Optional(
        properties.Integer, "version_number", serializable=False
    )
...
class FileCollection(Collection[FileLink]):
    ...
    def _get_path_from_file_link(
        self, file_link: FileLink, *, action: str = None
    ) -> str:
        ...
        return self._get_path(
            uid=file_link.uid, version=file_link.version, action=action
        )
    ...
    def get(
        self, uid: Union[UUID, str], *, version: Optional[Union[UUID, str, int]] = None
    ) -> FileLink:
        """
    ...
    def _make_upload_request(self, file_path: Path, dest_name: str):
        ...
        upload_json = {
            "files": [
                {"file_name": dest_name, "mime_type": mime_type, "size": file_size}
            ]
        }
        ...
        try:
            ...
            raise RuntimeError(
                "Upload initiation response is missing some fields: {}".format(
                    upload_request
                )
            )
        return uploader
    ...
    def _search_by_file_name(
        self, file_name: str, dset_id: UUID, file_version_number: Optional[int] = None
    ) -> Optional[FileLink]:
        """
    ...
    def _search_by_file_version_id(self, file_version_id: UUID) -> Optional[FileLink]:
        """
    ...
    def _upload_file(file_path: Path, uploader: _Uploader):
        ...
        with file_path.open(mode="rb") as f:
            try:
                ...
                raise RuntimeError(
                    f"Upload of file {file_path} failed with the following "
                    f"exception: {e}"
                )
    ...
    def _complete_upload(self, dest_name: str, uploader: _Uploader):
        ...
        try:
            ...
            raise RuntimeError(
                "Upload completion response is missing some fields: {}".format(
                    complete_response
                )
            )
    ...
    def download(
        self, *, file_link: Union[str, UUID, FileLink], local_path: Union[str, Path]
    ):
        """
    ...
    def read(self, *, file_link: Union[str, UUID, FileLink]) -> bytes:
        ...
        if self._is_local_url(file_link.url):  # Read the local file
            ...
            if parsed_url.netloc not in {"", ".", "localhost"}:
                raise ValueError(
                    "Non-local UNCs (e.g., Windows network paths) are not supported."
                )
        ...
        if self._is_external_url(file_link.url):  # Pull it from where ever it lives
            ...
            content_link = self._get_path_from_file_link(
                file_link, action="content-link"
            )
            content_link_response = self.session.get_resource(content_link)
            pre_signed_url = content_link_response["pre_signed_read_link"]
            final_url = rewrite_s3_links_locally(
                pre_signed_url, self.session.s3_endpoint_url
            )
    ...
    def ingest(
        ...
        *,
        ...
        def resolve_with_local(candidate: Union[FileLink, Path, str]) -> FileLink:
            ...
            return self._resolve_file_link(
                candidate
            )  # It was a FileLink or unresolvable string
        ...
        if upload:
            with TemporaryDirectory() as downloads:
                for file_link in offplatform:
                    ...
                    onplatform.append(
                        self.upload(file_path=path, dest_name=file_link.filename)
                    )
    ...
    def delete(self, file_link: FileLink):
        ...
        if not self._is_on_platform_url(file_link.url):
            raise ValueError(
                f"Only platform resources can be deleted; passed URL {file_link.url}"
            )
        file_id = _get_ids_from_url(file_link.url)[0]
        if file_id is None:
            raise ValueError(
                f"URL was malformed for local resources; passed URL {file_link.url}"
            )
            ...
            if not isinstance(
                ...
                if self._is_on_platform_url(update.url):
                    if update.uid is None:
                        raise ValueError(
                            f"URL was malformed for platform resources; "
