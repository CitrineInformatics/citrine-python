...
class DesignSpaceCollection(Collection[DesignSpace]):
    ...
    def _verify_write_request(self, design_space: DesignSpace):
        ...
        if isinstance(design_space, EnumeratedDesignSpace):
            warnings.warn(
                "As of 3.27.0, EnumeratedDesignSpace is deprecated in favor of a "
    ...
    def _verify_read_request(self, design_space: DesignSpace):
        ...
        if isinstance(design_space, EnumeratedDesignSpace):
            ...
            typ = type(design_space).__name__
            warnings.warn(
                f"As of 3.27.0, top-level {typ}s are deprecated. Any that remain when "
    ...
    def create_default(
        ...
        *,
        ...
        data = self.session.post_resource(
            path, json=settings.dump(), version=self._api_version
        )
    ...
    def convert_to_hierarchical(
        ...
        *,
        ...
        return HierarchicalDesignSpace.build(
            DesignSpace.wrap_instance(data["instance"])
        )
