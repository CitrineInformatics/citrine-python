...
class GemTable(Resource["Table"]):
    ...
    def __str__(self):
        return f"<GEM Table {self.uid!r}, version {self.version}>"
    ...
    def config(self) -> TableConfig:
        ...
        if self._config is None:
            config_collection = TableConfigCollection(
                team_id=self._team_id, project_id=self._project_id, session=self._session
            )
...
class GemTableCollection(Collection[GemTable]):
    ...
    def list_versions(self, uid: UUID, *, per_page: int = 100) -> Iterable[GemTable]:
        ...
        def _fetch_versions(page: int | None, per_page: int) -> tuple[Iterable[dict], str]:
    ...
    def list_by_config(self, table_config_uid: UUID, *, per_page: int = 100) -> Iterable[GemTable]:
        ...
        def _fetch_versions(page: int | None, per_page: int) -> tuple[Iterable[dict], str]:
            ...
            data = self.session.get_resource(path, params=self._page_params(page, per_page))
            return data[self._collection_key], data.get("next", "")
    ...
    def initiate_build(
        self, config: TableConfig | str | UUID, *, version: str | UUID = None
    ...
    def get_by_build_job(
        self, job: JobSubmissionResponse | UUID, *, timeout: float = 15 * 60
    ) -> GemTable:
        """
