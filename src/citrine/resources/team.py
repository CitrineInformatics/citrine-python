...
class TeamMember:
    ...
    def __str__(self):
        return f"<TeamMember {self.user.screen_name!r} can {self.actions!s} in {self.team.name!r}>"
...
class TeamResourceIDs:
    ...
    def __init__(self, session: Session, team_id: str | UUID, resource_type: str) -> None:
    ...
    def _list_ids(self, action: str) -> list[str]:
        ...
        return self.session.get_resource(
            f"/{self.resource_type}/authorized-ids", params=query_params, version=self._api_version
        )["ids"]
...
class Team(Resource["Team"]):
    ...
    def __init__(self, name: str, *, description: str = "", session: Session | None = None):
        self.name: str = name
    ...
    def add_user(
        self, user_id: str | UUID | User, *, actions: list[TEAM_ACTIONS] | None = None
    ) -> bool:
        """
    ...
    def update_user_action(
        self, user_id: str | UUID | User, *, actions: list[TEAM_ACTIONS]
    ) -> bool:
        """
    ...
    def share(self, *, resource: Resource, target_team_id: "str | UUID | Team") -> bool:
        """
    ...
    def dataset_ids(self) -> TeamResourceIDs:
        ...
        return TeamResourceIDs(
            session=self.session, team_id=self.uid, resource_type=ResourceTypeEnum.DATASET.value
        )
    ...
    def module_ids(self) -> TeamResourceIDs:
        ...
        return TeamResourceIDs(
            session=self.session, team_id=self.uid, resource_type=ResourceTypeEnum.MODULE.value
        )
    ...
    def table_ids(self) -> TeamResourceIDs:
        ...
        return TeamResourceIDs(
            session=self.session, team_id=self.uid, resource_type=ResourceTypeEnum.TABLE.value
        )
