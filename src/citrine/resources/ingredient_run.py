...
class IngredientRun(
    GEMDResource["IngredientRun"], ObjectRun, GEMDIngredientRun, typ=GEMDIngredientRun.typ
):
    """
    An ingredient run.
    ...
    def __str__(self):
        return f"<Ingredient run {self.name!r}>"
...
class IngredientRunCollection(ObjectRunCollection[IngredientRun]):
    ...
    def list_by_spec(
        self, uid: UUID | str | LinkByUID | GEMDIngredientSpec
    ...
    def list_by_process(
        self, uid: UUID | str | LinkByUID | GEMDProcessRun
    ) -> Iterator[IngredientRun]:
        """
