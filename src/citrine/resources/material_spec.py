...
class MaterialSpec(
    GEMDResource["MaterialSpec"], ObjectSpec, GEMDMaterialSpec, typ=GEMDMaterialSpec.typ
):
    """
    ...
    template = PropertyOptional(
        LinkOrElse(GEMDMaterialTemplate),
        "template",
        override=True,
        use_init=True,
    )
...
class MaterialSpecCollection(ObjectSpecCollection[MaterialSpec]):
    ...
    def get_by_process(
        ...
        """
        return next(
            self._get_relation(relation="process-specs", uid=uid, per_page=1), None
        )
