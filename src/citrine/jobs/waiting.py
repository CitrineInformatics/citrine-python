...
def wait_for_asynchronous_object(
    *,
    ...
    while not is_finished() and (time.time() - start < timeout):
        time.sleep(interval)
    if not is_finished():
        raise ConditionTimeoutError("Timeout of {timeout_length} seconds reached, but task {uid} is still in progress".format(timeout_length=timeout, uid=resource.uid))
